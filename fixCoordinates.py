...
if len(sys.argv) != 4:
    print( "Inp template, inp 2 fix, fixed name")
else:
    template = sys.argv[1]
    inp2fix = sys.argv[2]
    inpFixed = sys.argv[3]

    indexes, coords = getFrozenIndexes(inp2fix)
    indexes2, coords2 = getFrozenIndexes(template)
    ...
    writeNewInput (inp2fix, newCoords1 ,  inpFixed )
